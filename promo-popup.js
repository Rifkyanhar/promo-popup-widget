 ...
 *
 * Widget ini otomatis ganti isi pesan sesuai tanggal:
 *  - Tanggal 1–15   : promo OTP (One Time Payment / sekali bayar)
 *  - Tanggal 16–akhir bulan : ajakan coba dulu via langganan bulanan
 *
 * CARA PAKAI:
 ...
 *
 * Kenapa toggle terpisah dari kode? Supaya kamu bisa mematikan popup
 * kapan saja tanpa utak-atik kode — misalnya kalau kuota 5 pengguna/hari
 * sudah penuh dan kamu tidak mau iklankan promo lagi hari itu.
 */
(function () {
  "use strict";
  ...
  var CONFIG = {
    ...
    // Batas tanggal periode promo OTP (di luar rentang ini otomatis
    // dianggap periode langganan trial)
    otpStartDay: 1,
    otpEndDay: 15,
  };
  ...
  var VARIANTS = {
    otp: {
      eyebrow: "🔥 Promo Bulanan Aktif",
      title: 'Diskon Spesial <span>Tgl 1–15</span> Bulan Ini!',
      desc: "Absensi Guru & Absensi Digital Instansi mulai Rp300rb, Sistem Pesan Menu Resto QR mulai Rp400rb — bayar sekali (OTP), langsung pakai. Kuota harian terbatas!",
      linkUrl: "https://nafatech.web.id/promo",
      linkLabel: "Lihat Syarat & Promo",
      countdown: function () {
        var left = daysLeftUntilDay(CONFIG.otpEndDay);
        return left > 0 ? "⏳ Promo OTP berakhir dalam " + left + " hari lagi" : "⏳ Promo OTP segera berakhir";
      },
    },
    trial: {
      eyebrow: "🎁 Coba Gratis Dulu",
      title: "Coba Semua Fitur, <span>Bayar Kalau Cocok</span>",
      desc: "Tanggal 16 sampai akhir bulan, coba dulu Absensi Digital Instansi, Absensi Guru & Sistem Pesan Menu Resto QR lewat langganan bulanan. Cocok? Upgrade ke sekali bayar (OTP) di tanggal 1–15 bulan berikutnya.",
      linkUrl: "https://nafatech.web.id/promo",
      linkLabel: "Lihat Detail Langganan",
      countdown: function () {
        var left = daysLeftUntilEndOfMonth();
        return left > 0 ? "⏳ Periode coba/langganan berakhir dalam " + left + " hari lagi" : "⏳ Periode coba/langganan segera berakhir";
      },
    },
  };
  // =======================================================================
  ...
  function getActiveVariant() {
    ...
    return (day >= CONFIG.otpStartDay && day <= CONFIG.otpEndDay) ? VARIANTS.otp : VARIANTS.trial;
  }

  function daysLeftUntilDay(targetDay) {
    var now = new Date();
    var end = new Date(now.getFullYear(), now.getMonth(), targetDay, 23, 59, 59);
  ...
  function buildPopup() {
    ...
    wrap.innerHTML =
      ...
      '<div class="nftp-body">' +
        ...
        '<div class="nftp-countdown">' + variant.countdown() + '</div>' +
        '<div class="nftp-ctas">' +
