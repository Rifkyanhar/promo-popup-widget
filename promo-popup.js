...
(function () {
  ...
  var CONFIG = {
    ...
    // Link tujuan tombol utama
    promoLinkUrl: "https://nafatech.web.id/promo",
  ...
  function withinPromoWindow() {
    if (!CONFIG.restrictToPromoDates) return true;
    var day = new Date().getDate();
    return day >= CONFIG.promoStartDay && day <= CONFIG.promoEndDay;
  }
  ...
  function daysLeftInPromo() {
    var now = new Date();
    var end = new Date(now.getFullYear(), now.getMonth(), CONFIG.promoEndDay, 23, 59, 59);
  ...
  function buildPopup() {
    ...
    wrap.setAttribute("aria-label", "Promo NAFA Tech");

    var left = daysLeftInPromo();
    var countdownText = left > 0
      ? "⏳ Promo berakhir dalam " + left + " hari lagi"
      : "⏳ Promo bulan ini segera berakhir";

    wrap.innerHTML =
      '<div class="nftp-glow"></div>' +
      '<button class="nftp-close" aria-label="Tutup">✕</button>' +
      '<div class="nftp-body">' +
        '<span class="nftp-eyebrow">🔥 Promo Bulanan Aktif</span>' +
        '<div class="nftp-title">Diskon Spesial <span>Tgl 1–15</span> Bulan Ini!</div>' +
        '<div class="nftp-desc">Absensi Guru & Absensi Digital Instansi mulai Rp300rb, Sistem Pesan Menu Resto QR mulai Rp400rb. Kuota harian terbatas!</div>' +
        '<div class="nftp-countdown">' + countdownText + '</div>' +
        '<div class="nftp-ctas">' +
          '<a class="nftp-btn nftp-btn-primary" href="' + CONFIG.promoLinkUrl + '" target="_blank" rel="noopener">Lihat Syarat & Promo</a>' +
  ...
  // berfungsi selama masih dalam masa promo.
